'use strict';

export default [
  {
    type: 'Web page',
    title: 'Interactive resume',
    link: 'jankowskiresume.com',
    description: `As you are aware, this is the page you're currently browsing. It's objective is to be a small show off
      of few of my front-end skills and nicer presentation of my resume. I also always wanted to craft a pixel-art,
      game-like page. Based on React and Redux.`
  },
  {
    type: 'Game',
    title: 'The Space Rabbit',
    link: 'thespacerabbit.com',
    description: `A dexterity based game made in Unity3D, written in C#. Handmade graphics (with a bit of FontAwesome)
      in Inkscape. Your objective is to feed the Mighty Rabbit and score as many points as possible.`
  },
  {
    type: 'Mobile app',
    title: 'Tiny Life Counter',
    link: 'tinyurl.com/zm32bb8',
    description: `Since I am a Magic The Gathering player, I decided to make this hybrid application. I was disappointed
      by currently available life counters, due to their terrible battery usage. This tiny hero is easy on power usage
      and provides basic functions for everyday MTG games. Base technologies used: Apache Cordova (wrapper), ReactJS,
      React-Router, Redux, Ramda.`
  }
];
